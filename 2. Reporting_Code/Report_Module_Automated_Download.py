...
class PortfolioReportingFrameworkDownload:
    """
    ...
    def _setup_directories(self):
        ...
        self.ibstatement_dir = os.path.join(self.ghfm_reporting_dir, "1. Reporting_Data/IB_Daily Statement/")
        ...
        self.ibtrades_dir = os.path.join(self.ghfm_reporting_dir, "1. Reporting_Data/IB_Trades/")
        ...
        self.currencyrates_dir = os.path.join(self.ghfm_reporting_dir, "1. Reporting_Data/Currency_Rate/")
        
    # ======================== PERFORMANCE REPORTING METHODS ========================
    
    def calculate_nav(self, csv_file):
        """Extract NAV from CSV file."""
        current_total_nav = None
        with open(csv_file, 'r', encoding='utf-8') as file:
            csv_reader = csv.reader(file)
            for row in csv_reader:
                if 'Net Asset Value' in row[0] and 'Total' in row[2]:
                    current_total_nav = float(row[6])
                    break
        return current_total_nav

    def calculate_cash(self, csv_file):
        """Extract cash injection from CSV file."""
        cash = 0.0
        with open(csv_file, 'r', encoding='utf-8') as file:
            csv_reader = csv.reader(file)
            for row in csv_reader:
                if len(row) < 4:
                    continue
                if 'Change in NAV' in row[0] and 'Deposits & Withdrawals' in row[2]:
                    try:
                        cash = float(row[3])
                    except ValueError:
                        cash = 0.0
    ...
    def compute_rolling_metrics(self, df_hist):
        ...
        # CORRECTED: Trailing 365-day metrics
        ...
        # Option 1: Using trading days (recommended if each row represents a trading day)
        n_trading_days = len(df_hist)
        df_hist.at[last_idx, "CAGR"] = df_hist.at[last_idx, "Cal"]**(252 / n_trading_days) - 1
        
        # Option 2: Using calendar days (uncomment if you prefer calendar-based CAGR)
        # n_calendar_days = (today_date - df_hist.iloc[0]["DATE"]).days + 1
        # df_hist.at[last_idx, "CAGR"] = df_hist.at[last_idx, "Cal"]**(365.25 / n_calendar_days) - 1
    ...
    def run_performance_report(self):
        ...
        csv_file = os.path.join(self.ibstatement_dir, self.year_str, self.month_str, f"{self.ibaccount_str}_{self.today_str}.csv")

        today_nav = self.calculate_nav(csv_file)
        today_cash = self.calculate_cash(csv_file)
    ...
    def add_market_value_usd(self, df):
        ...
        currency_file_path = os.path.join(self.currencyrates_dir, self.year_str, self.month_str, f"Currency_rate_{self.today_str}.csv")
        #currency_file_path = os.path.join(self.currencyrates_dir, "2025", "202509", "Currency_rate_20250909.csv")
        with open(currency_file_path, "r", encoding="utf-8-sig") as f:
            for i, line in enumerate(f):
                # Remove leading/trailing quotes and whitespace
                clean_line = line.strip().strip('"')
                if clean_line.startswith("Date/Time"):
                    header_row = i
                    break

        df_rates = pd.read_csv(currency_file_path, skiprows=header_row)
        rate_map = df_rates[df_rates['ToCurrency'] == 'USD'].set_index('FromCurrency')['Rate'].to_dict()
    ...
    def extract_mtm(self):
        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            csv_reader = csv.reader(file)
            for row in csv_reader:
                if not start_reading and row and 'Mark-to-Market Performance Summary' in row[0]:
                    start_reading = True
                if start_reading:
                    matching_rows.append(row)

        data_dicts = [dict(zip(matching_rows[0], row)) for row in matching_rows if row]
        data_dicts = [d for d in data_dicts if d]
        df = pd.DataFrame(data_dicts)

        df = df[df['Mark-to-Market Performance Summary'] == 'Mark-to-Market Performance Summary']
        df = df[~df['Asset Category'].isin([
            'Forex', 'Total', 'Total (All Assets)', 'Broker Interest Paid and Received', 'Other Fees'
        ])]

        df = df[['Symbol', 'Current Quantity', 'Current Price', 'Mark-to-Market P/L Total']]
        df = df.set_index('Symbol').T.drop('Symbol', axis=1)
        df['Date'] = self.today_str
        
        tidy_df = df.drop(columns=['Date']).T.reset_index()
        tidy_df.columns = ['Symbol', 'Current Quantity', 'Current Price','MTM P&L']
        tidy_df['Current Price'] = tidy_df['Current Price'].str.replace(",", "", regex=False)

        bad_mask = ~tidy_df['Current Price'].astype(str).str.replace(",", "").str.replace(".", "").str.isnumeric()
        if bad_mask.any():
            print("Warning: Non-numeric prices found:")
            print(tidy_df.loc[bad_mask, ['Symbol', 'Current Price']])

        tidy_df['Current Price'] = pd.to_numeric(tidy_df['Current Price'], errors='coerce').fillna(0).round(4)
        tidy_df['Current Quantity'] = pd.to_numeric(tidy_df['Current Quantity'], errors='coerce')
        tidy_df['MTM P&L'] = pd.to_numeric(tidy_df['MTM P&L'], errors='coerce').round(2)
        tidy_df["Market Value FCY"] = tidy_df["Current Quantity"] * tidy_df["Current Price"]
        tidy_df['Market Value FCY'] = pd.to_numeric(tidy_df['Market Value FCY'], errors='coerce').round(2)
        ...
        #ibkr_file = os.path.join(self.ib_ticker_dir, "ibkr", "2025", "202509", "IB_Ticker_20250909.csv")
        ...
        today_tickers = [col for col in df.columns if col != 'Date']
    ...
    def process_trades(self):
        ...
        csv_file = os.path.join(self.ibtrades_dir, self.year_str, self.month_str, f"Trades_{self.ibaccount_str}_{self.today_str}.csv")
        ...
        df = pd.DataFrame(matching_rows, columns=cols)
        df = df[~df['Header'].isin(['SubTotal', 'Total', 'Header'])]

        for col in df.columns:
            series = df[col]
            # If multiple columns exist with same name, take the first one
            if isinstance(series, pd.DataFrame):
                series = series.iloc[:, 0]
            if series.dtype == 'object':
                df[col] = series.astype(str).str.replace(",", "")


        df = df[['Symbol', 'T. Price', 'Quantity']]
        df['T. Price'] = pd.to_numeric(df['T. Price'], errors='coerce')
        df['Quantity'] = pd.to_numeric(df['Quantity'], errors='coerce')
        df['TotalValue'] = df['T. Price'] * df['Quantity']

        grouped_df = df.groupby('Symbol').agg({'T. Price': 'mean', 'Quantity': 'sum', 'TotalValue': 'sum'}).reset_index()
        if grouped_df.empty:
            grouped_df = pd.DataFrame(columns=['Symbol', 'Quantity', 'AvgPrice'])
        else:
            grouped_df['AvgPrice'] = grouped_df.apply(
                lambda row: np.nan if row['Quantity'] == 0 else row['TotalValue'] / row['Quantity'], axis=1
            ).round(2)
            grouped_df = grouped_df[['Symbol', 'Quantity', 'AvgPrice']]
    ...
    def run_daily_report(self):
        ...
        return {
            ...
            'prior_total_nav': prior_total_nav,
            ...
            'merged_df': merged_df,
            'market_value_df': market_value_df
        }
    ...
    def run_complete_daily_report(self):
        ...
        # Step 1: Run performance reporting first
        print("Step 1: Running performance reporting...")
        performance_df = self.run_performance_report()
        print("Performance reporting completed.\n")
        
        # Step 2: Run daily reporting (uses performance results)
        print("Step 2: Running daily reporting...")
        daily_results = self.run_daily_report()
