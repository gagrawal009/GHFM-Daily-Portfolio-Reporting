...
def get_previous_weekday():
    ...
    # If it's a weekend, keep going back until we find a weekday
    while previous_day.weekday() > 4:  # 0-4 is Monday-Friday, 5-6 is Saturday-Sunday
...
def clean_dataframe_for_display(df):
    ...
    # Remove rows where all values are empty strings, whitespace, or NaN
    df_clean = df_clean.replace('', pd.NA)  # Replace empty strings with NaN
    df_clean = df_clean.dropna(how='all')  # Drop rows that are all NaN
    
    # Remove rows where all non-null values are just whitespace
...
def create_performance_tables(selected_datetime, perf_df, mv_df, pnl_df):
    ...
    # Performance by Asset Class - Restructured
    ...
    for asset_class in asset_classes:
        ...
        # Extract number of tickers from count columns
        ...
        performance_data[asset_class] = [
            str(int(ticker_count)),  # Convert to string to avoid Arrow issues
    ...
    performance_data['Total'] = [
        str(total_tickers),  # Convert to string to avoid Arrow issues
    ...
    # Monthly and Yearly Attribution
...
def create_plotly_charts(performance_by_asset_class, monthly_attrib_usd, asset_classes, perf_df, selected_datetime):
    ...
    if month_str in monthly_attrib_usd.index:
        ...
        # Calculate dynamic y-axis range to ensure text labels are visible
        max_val = max(monthly_pnl) if len(monthly_pnl) > 0 else 1000
        min_val = min(monthly_pnl) if len(monthly_pnl) > 0 else -1000
        y_range_padding = max(abs(max_val), abs(min_val)) * 0.15  # 15% padding
...
def main():
    ...
    # Add note about calculation methodology
    st.markdown(
    ...
    # Performance by Asset Class - Enhanced Layout
    ...
    # Toggle button to show/hide detailed performance for all symbols
    if not all_symbol_pnl_df.empty:
        ...
        if st.session_state.show_all_symbols:
            ...
            # Clean the dataframe and remove index
    ...
    # Add Total column to the original dataframe
    ...
    # Toggle button to show/hide PnL of Asset Categories day by day
    # Dynamic button text based on current state
    ...
    if st.session_state.show_pnl_daily:
        ...
        # Create a formatted version of pnl_df for better display
        pnl_display_df = pnl_df.copy()
        # Format numeric columns for currency display
    ...
    if not trades_df.empty:
        # Clean the trades dataframe - remove empty rows and convert for Arrow compatibility
